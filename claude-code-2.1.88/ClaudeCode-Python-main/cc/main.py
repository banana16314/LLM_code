...
def _get_base_url() -> str | None:
    """Get base url from environment variable or project .env file.
...
def _get_model_name() -> str | None:
    """Get served model name from environment variable or project .env file.

    Priority:
    1. MODEL environment variable
    2. .env file in project root (cc-python-claude/)
    """
...
async def _run_print_mode(prompt: str, model: str, base_url: str) -> None:
...
async def _run_repl(model: str, base_url: str, resume_id: str | None = None) -> None:
...
@click.option("--model", default="Qwen", help="Model to use") #claude-sonnet-4-20250514
@click.option("--base_url", default="http://0.0.0.0:8000/v1", help="base url to use") #claude-sonnet-4-20250514
...
def main(
    ...
    if not model:
        console.print("[red]Error: No model found. Set MODEL env var or add it to .env file.[/]")
        sys.exit(1)

    base_url = _get_base_url()
    if not base_url:
        console.print("[red]Error: No base_url found. Set BASE_URL env var or add it to .env file.[/]")
        sys.exit(1)
    logger.info("claude code using: model-{model}, base_url-{base_url}")
    if print_mode:
        if not prompt:
        ...
        if not prompt:
            ...
            sys.exit(1)
        asyncio.run(_run_print_mode(prompt, model, base_url))
    else:
        asyncio.run(_run_repl(model, base_url, resume_id=resume_id))
