...
# 【核心修改】：由于我们的镜像没有使用 venv，这里直接调用系统全局的 python3
DEFAULT_CMD_1="python3"
...
start_internal_nginx_mode() {
  ...
  # ======= 【核心修复 1】：自动生成临时 SSL 证书以支持 HTTPS =======
  echo "[entrypoint] Generating self-signed SSL certificate for HTTPS..."
  openssl req -x509 -nodes -days 3650 -newkey rsa:2048 -keyout /tmp/nginx.key -out /tmp/nginx.crt -subj "/CN=funasr-api" 2>/dev/null
  # =================================================================

  {
    ...
    # ======= 【核心修复 2】：为 Nginx 增加 HTTPS 端口及证书配置 =======
    echo "        listen 8443 ssl;"
    echo "        ssl_certificate /tmp/nginx.crt;"
    echo "        ssl_certificate_key /tmp/nginx.key;"
    # ==================================================================
    echo "        server_name _;"
  ...
  echo "[entrypoint] Starting internal nginx load balancer on HTTP :${public_port} and HTTPS :8443"
